...
interface CreatePackSheetTabbedProps {
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
  categories: Category[]
...
export function CreatePackSheetTabbed({ isOpen, onClose, onSuccess, categories = [] }: CreatePackSheetTabbedProps) {
  const { user } = useAuth()
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (selectedServices.length > 0) {
        ...
        await supabase.from('pack_services').insert(serviceInserts)
      }

      // 3. Ajouter l'itinéraire
      if (itinerary.length > 0) {
        ...
        await supabase.from('pack_itinerary').insert(itineraryInserts)
      }
      ...
      if (validImages.length > 0) {
        ...
        await supabase.from('pack_images').insert(imageInserts)
      }
    ...
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent side="right" className="w-[800px] p-0 flex flex-col max-h-screen">
        <SheetHeader className="px-6 py-4 border-b shrink-0">
          <SheetTitle>Créer un nouveau pack</SheetTitle>
          <SheetDescription>
            Ajoutez un nouveau pack touristique en suivant les étapes
          </SheetDescription>
        </SheetHeader>
